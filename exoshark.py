...
elif action == 'changelogtxt':
    from resources.lib.modules import es_utils
    es_utils.changelog_textbox()

elif action == 'subtCheck':
    from resources.lib.modules import es_utils
    es_utils.check_subtitle(tvshowtitle, year, season, episode)

elif action == 'movieRls':
    from resources.lib.modules import es_utils
    es_utils.movierls(title, year)

elif action == 'imdbInfo':
    from resources.lib.modules import es_utils
    es_utils.imdbinfo(imdb)

elif action == 'refreshProviders':
    from resources.lib.modules import control
    from resources.lib.modules import es_utils
    es_utils.refreshproviders()
    dialogtext = u'A friss\u00EDt\u00E9s befejez\u0151d\u00F6tt. R\u00E9szletek a hibanapl\u00F3ban.'
    control.infoDialog(dialogtext.encode('utf-8'), sound=True, icon='INFO')

elif action == 'importProviders':
    from resources.lib.modules import es_utils
    type = params.get('type')
    es_utils.importproviders(type)

elif action == 'checkProviders':
    from resources.lib.modules import es_utils
    es_utils.checkproviders()

elif action == 'providerTest':
    from resources.lib.modules import control
    if control.setting('test.providers') == 'true':
        control.setSetting('test.providers', 'false')
        dialogtext = u'Kiszolg\u00E1l\u00F3 tesztel\u00E9s kikapcsolva'
    else:
        control.setSetting('test.providers', 'true')
        dialogtext = u'Kiszolg\u00E1l\u00F3 tesztel\u00E9s bekapcsolva'
    control.infoDialog(dialogtext.encode('utf-8'), sound=True, icon='INFO')

elif action == 'devTools':
    from resources.lib.indexers import navigator
    from resources.lib.modules import control
    passw = 'ZXhvc2hhcmsxMjM='
    t = u'Fejleszt\u0151i eszk\u00F6z\u00F6k jelsz\u00F3'
    k = control.keyboard('', t.encode('utf-8'))
    k.doModal()
    c = k.getText() if k.isConfirmed() else None
    if not c == passw.decode('base64'):
        control.infoDialog(u'Hib\u00E1s jelsz\u00F3', sound=True, icon='INFO')
    else:
        navigator.navigator().dev_tools()

elif action == 'dialogText':
    from resources.lib.modules import es_utils
    name = params.get('name')   
    txt = params.get('txt')
    es_utils.dialogtext(name, txt)
    
elif action == 'dialogrepo':
    from resources.lib.modules import es_utils
    es_utils.dialogrepo()
