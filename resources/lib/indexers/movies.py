...
class movies:
    def __init__(self):
        ...
        if self.tm_user == '': self.tm_user = '3b3eccd1c6bc0f679223fdbd7cf0d06d'
        ...
        self.tm_art_link = 'http://api.themoviedb.org/3/movie/%s/images?api_key=%s&language=en-US&include_image_language=hu,en,null' % ('%s', self.tm_user)
        ...
        self.latestreleases_link = 'http://api.trakt.tv/users/giladg/lists/latest-releases/items'
        self.toprated_link = 'http://api.trakt.tv/users/justin/lists/imdb-top-rated-movies/items'
    ...
    def movieDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                cm.append((u'Hasonl\u00F3 keres\u00E9se',
                           'ActivateWindow(10025,%s?action=movies&url=https://api.trakt.tv/movies/%s/related,return)' % (
                           sysaddon, imdb)))

                cm.append((queueMenu, 'RunPlugin(%s?action=queueItem)' % sysaddon))

                cm.append((u'Megjelen\u00E9sek', 'RunPlugin(%s?action=movieRls&title=%s&year=%s)' % (sysaddon, systitle, year)))
				
                cm.append((u'IMDb Inf\u00F3', 'RunPlugin(%s?action=imdbInfo&imdb=%s)' % (sysaddon, imdb)))
