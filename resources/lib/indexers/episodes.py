...
class episodes:
        ...
        for item in items:
            try:
                ...
                try:
                    ...
                    if url == self.onDeck_link or '/calendars/my/shows/' in url or '/users/me/history/' in url: tvshowtitle = tvshowtitle
                    else: tvshowtitle = trakt.getTVShowTranslation(imdb, lang=self.lang) or tvshowtitle
                except:
                    pass
    ...
    def episodeDirectory(self, items):
        ...
        subtitleCheck = u'Felirat keres\u00E9se (SuperSubtitles)'.encode('utf-8') ###
        ...
        for i in items:
            try:
                ...
                cm.append((playRandom, 'RunPlugin(%s?action=random&rtype=episode&tvshowtitle=%s&year=%s&imdb=%s&tvdb=%s&season=%s)' % (sysaddon, systvshowtitle, year, imdb, tvdb, season)))

                cm.append((queueMenu, 'RunPlugin(%s?action=queueItem)' % sysaddon))

                cm.append((subtitleCheck, 'RunPlugin(%s?action=subtCheck&tvshowtitle=%s&year=%s&season=%s&episode=%s)' % (sysaddon, systvshowtitle, year, season, episode)))
