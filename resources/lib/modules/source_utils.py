...
def getFileType(url):
    ...
    if any(i in url for i in ['bluray', 'blu-ray']):
        type += ' BLURAY /'
    if any(i in url for i in ['.web-dl', '.webdl.', '.web.', 'webrip', 'web-rip']):
        type += ' WEB-DL /'
    if any(i in url for i in ['uhdrip']):
        type += ' UHDRip /'
    if any(i in url for i in ['hdrip']) and not 'uhdrip' in url:
        type += ' HDRip /'
    if any(i in url for i in ['bd-r', 'bd.r', 'bdr']):
        type += ' BD-R /'
    if any(i in url for i in ['bd-rip', 'bd.rip', 'bdrip']):
        type += ' BD-RIP /'
    if any(i in url for i in ['brrip']):
        type += ' BRRip /'
    if any(i in url for i in ['atmos']):
        type += ' ATMOS /'
    if any(i in url for i in ['truehd']):
        type += ' TRUEHD /'
    if any(i in url for i in ['.dd']):
        type += ' DolbyDigital /'
    if any(i in url for i in ['5.1']):
        type += ' 5.1 /'
    if any(i in url for i in ['.xvid']):
        type += ' XVID /'
    if any(i in url for i in ['.mp4']):
        type += ' MP4 /'
    if any(i in url for i in ['.avi']):
        type += ' AVI /'
    if any(i in url for i in ['ac3']):
        type += ' AC3 /'
    if any(i in url for i in ['h.264', 'h264', 'x264']):
        type += ' H.264 /'
    if any(i in url for i in ['h.264', 'h265', 'x265']):
        type += ' H.265 /'
    if any(i in url for i in ['.hdr.']):
        type += ' HDR /'
    if any(i in url for i in ['subs']):
        if type != '': type += ' - FELIRATTAL'
        else: type += 'FELIRAT'
    type = type.rstrip('/')
