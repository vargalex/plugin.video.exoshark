...
import sys,re,json,urllib,urlparse,random,datetime,time,os
...
if control.setting('resolver.url') == '' or control.setting('resolver.url') == '0':
...
class sources:
        ...
        try:
            ...
            for i in range(len(items)):
                try:
                    ...
                    offset = 60 * 2 if items[i].get('source') in self.hostcapDict or items[i].get('sourcecap') == True else 0
    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, timeout=30):
        ...
        try:
            if os.path.isfile(os.path.join(control.dataPath, 'rdcache.v')):
                control.deleteFile(os.path.join(control.dataPath, 'rdcache.v'))
        except:
            pass
        if control.setting('test.providers') == 'true':
            try:
                from resources.lib.modules import es_utils
                selectedproviders = es_utils.testproviders()
                sourceDict = [i for i in sourceDict if i[0] in selectedproviders] # testing providers
            except:
                pass
        ...
        if content == 'movie':
            ...
            #Disabled due to hang. Should be checked in the future and possible enabled again.
            ...
            try:
                ...
                if (i / 2) < timeout:
                    ...
                    try:
                        ...
                        info = mainleft
                        if debrid_status:
                            if len(info) > 6: line3 = '%s: %s' % (string5, str(len(info)))
                            elif len(info) > 0: line3 = '%s: %s' % (string5, ', '.join(info))
                            else: break
                            percent = int(100 * float(i) / (2 * timeout) + 0.5) % 100
                            if not progressDialog == control.progressDialogBG: progressDialog.update(max(1, percent), line1, line2, line3)
                            else: progressDialog.update(max(1, percent), line1, line3)
                        else:
                            if len(info) > 6: line2 = '%s: %s' % (string5, str(len(info)))
                            elif len(info) > 0: line2 = '%s: %s' % (string5, ', '.join(info))
                            else: break
        ...
        stats = control.setting('statistics')
        if stats == 'true':
            try:
                if content == 'movie':
                    con_title = self.getTitle(title)
                    con_localtitle = self.getLocalTitle(title, imdb, tvdb, content)
                    con_hdlr = year
                else:
                    con_title = self.getTitle(tvshowtitle)
                    con_localtitle = self.getLocalTitle(tvshowtitle, imdb, tvdb, content)
                    con_hdlr = 'S%02dE%02d | %s' % (int(season), int(episode), year)

                stat_string = '%s | %s | %s | %s' % (con_localtitle, con_title, con_hdlr, imdb)

                log_utils.log(stat_string, log_utils.LOGNOTICE)
            except:
                pass
    ...
    def sourcesFilter(self):
        ...
        camfilter = control.setting('camfilter')
        ...
        for inf in self.sources:
            if not 'info' in inf: inf.update({'info': ''})

        filter = []
        filter += local

        if quality in ['0']: filter += [i for i in self.sources if i['quality'] == '4K' and 'cached' in i]
        if quality in ['0']: filter += [i for i in self.sources if i['quality'] == '4K' and not 'cached' in i and 'debrid' in i]
        if quality in ['0']: filter += [i for i in self.sources if i['quality'] == '4K' and not 'cached' in i and not 'debrid' in i and 'memberonly' in i]
        if quality in ['0']: filter += [i for i in self.sources if i['quality'] == '4K' and not 'cached' in i and not 'debrid' in i and not 'memberonly' in i]

        if quality in ['0', '1']: filter += [i for i in self.sources if i['quality'] == '1440p' and 'cached' in i]
        if quality in ['0', '1']: filter += [i for i in self.sources if i['quality'] == '1440p' and not 'cached' in i and 'debrid' in i]
        if quality in ['0', '1']: filter += [i for i in self.sources if i['quality'] == '1440p' and not 'cached' in i and not 'debrid' in i and 'memberonly' in i]
        if quality in ['0', '1']: filter += [i for i in self.sources if i['quality'] == '1440p' and not 'cached' in i and not 'debrid' in i and not 'memberonly' in i]

        if quality in ['0', '1', '2']: filter += [i for i in self.sources if i['quality'] == '1080p' and 'cached' in i]
        if quality in ['0', '1', '2']: filter += [i for i in self.sources if i['quality'] == '1080p' and not 'cached' in i and 'debrid' in i]
        if quality in ['0', '1', '2']: filter += [i for i in self.sources if i['quality'] == '1080p' and not 'cached' in i and not 'debrid' in i and 'memberonly' in i]
        if quality in ['0', '1', '2']: filter += [i for i in self.sources if i['quality'] == '1080p' and not 'cached' in i and not 'debrid' in i and not 'memberonly' in i]

        if quality in ['0', '1', '2', '3']: filter += [i for i in self.sources if i['quality'] == '720p' and 'cached' in i]
        if quality in ['0', '1', '2', '3']: filter += [i for i in self.sources if i['quality'] == '720p' and not 'cached' in i and 'debrid' in i]
        if quality in ['0', '1', '2', '3']: filter += [i for i in self.sources if i['quality'] == '720p' and not 'cached' in i and not 'debrid' in i and 'memberonly' in i]
        if quality in ['0', '1', '2', '3']: filter += [i for i in self.sources if i['quality'] == '720p' and not 'cached' in i and not 'debrid' in i and not 'memberonly' in i]
        ...
        filter += [i for i in self.sources if i['quality'] in ['SD', 'SCR', 'CAM'] and 'cached' in i and 'debrid' in i]
        filter += [i for i in self.sources if i['quality'] in ['SD', 'SCR', 'CAM'] and not 'cached' in i]
        self.sources = filter
        ...
        if multi == False and self.sources[0]['language'] == 'hu':
            filter = []
            filter += [i for i in self.sources if not i['quality'] in ['SD', 'CAM', 'SCR'] and 'szinkron' in i['info']]
            filter += [i for i in self.sources if not i['quality'] in ['SD', 'CAM', 'SCR'] and not 'szinkron' in i['info']]
            filter += [i for i in self.sources if i['quality'] == 'SD' and 'szinkron' in i['info']]
            filter += [i for i in self.sources if i['quality'] in ['CAM', 'SCR'] and 'szinkron' in i['info']]
            filter += [i for i in self.sources if i['quality'] == 'SD' and not 'szinkron' in i['info']]
            filter += [i for i in self.sources if i['quality'] in ['CAM', 'SCR'] and not 'szinkron' in i['info']]
            self.sources = filter
        if multi == True:
            filter = []
            filter += [i for i in self.sources if i['language'] == 'hu' and not i['quality'] in ['SD', 'CAM', 'SCR'] and 'szinkron' in i['info']]
            filter += [i for i in self.sources if i['language'] == 'hu' and not i['quality'] in ['SD', 'CAM', 'SCR'] and not 'szinkron' in i['info']]
            filter += [i for i in self.sources if not i['language'] == 'hu' and not i['quality'] in ['SD', 'CAM', 'SCR']]
            filter += [i for i in self.sources if i['language'] == 'hu' and i['quality'] == 'SD' and 'szinkron' in i['info']]
            filter += [i for i in self.sources if i['language'] == 'hu' and i['quality'] in ['CAM', 'SCR'] and 'szinkron' in i['info']]
            filter += [i for i in self.sources if i['language'] == 'hu' and i['quality'] == 'SD' and not 'szinkron' in i['info']]
            filter += [i for i in self.sources if not i['language'] == 'hu' and i['quality'] == 'SD']
            filter += [i for i in self.sources if i['language'] == 'hu' and i['quality'] in ['CAM', 'SCR'] and not 'szinkron' in i['info']]
            filter += [i for i in self.sources if not i['language'] == 'hu' and i['quality'] in ['CAM', 'SCR']]
            self.sources = filter
        ...
        hdub_identify = control.setting('hdub.identify')
        if hdub_identify == '': hdub_identify = 'FF008000'
        hdub_identify = self.getHdubColor(hdub_identify)
        ...
        for i in range(len(self.sources)):
            ...
            try:
                if 'szinkron' in self.sources[i]['info']:
                    f = (' | '.join(['[I]%s [/I]' % info.strip() for info in self.sources[i]['info'].split('|')]))
                    f = ('[COLOR %s]' % (hdub_identify)) + f + '[/COLOR]'
                else:
                    f = (' | '.join(['[I]%s [/I]' % info.strip() for info in self.sources[i]['info'].split('|')]))
            except:
                f = ''
            ...
            if t:
                if q in ['4K', '1440p', '1080p', '720p']: label += '%s | [B][I]%s [/I][/B] | %s | [I]%s[/I]' % (s, q, f, t)
                elif q == 'SD': label += '%s | %s | [I]%s[/I]' % (s, f, t)
                else: label += '%s | [I]%s [/I] | %s | [I]%s[/I]' % (s, q, f, t)
            else:
                if q in ['4K', '1440p', '1080p', '720p']: label += '%s | [B][I]%s [/I][/B] | %s' % (s, q, f)
                elif q == 'SD': label += '%s | %s' % (s, f)
                else: label += '%s | [I]%s [/I] | %s' % (s, q, f)
            label = label.replace('| 0 |', '|').replace(' | [I]0 [/I]', '')
            label = label.replace('[I]HEVC [/I]', 'HEVC')
            label = re.sub('\[COLOR \w+\]\[I\]\s+\[/I\]\[/COLOR\]', ' ', label)
            ...
            if s.lower() == 'pmcached' or s.lower() == 'rdcached':
                self.sources[i]['label'] = '[COLOR orange]' + label.upper() + '[/COLOR]'
        ...
        try: 
            if not camfilter == 'true': self.sources = [i for i in self.sources if not 'CAM' in str(i['quality'])]
        except: pass
        ...
        if stats == 'true':
            try:
                ...
                for i in self.sources:
                    if i['quality'] == '720p': count_720 += 1
                    elif i['quality'] == '1080p': count_1080 += 1
                    else: count_sd += 1

                stats1 = 'TALALATI STATISZTIKA [ 1080P: %s | 720P: %s | SD: %s | OSSZES: %s ]' % (count_1080, count_720, count_sd, count_1080 + count_720 + count_sd)
                log_utils.log(stats1, log_utils.LOGNOTICE)
            except:
                pass

            try:
                from collections import Counter
                provider_name = [i['provider'] for i in self.sources]
                countDict = Counter(provider_name)
                countDict = dict(countDict)
                providersCount = []
                for i in countDict:
                    providersCount.append('%s: %s' % (i.upper(), countDict[i]))

                stats2 = ' | '.join(providersCount)
                stats2 = 'TALALATI STATISZTIKA [ %s ]' % stats2
                log_utils.log(stats2, log_utils.LOGNOTICE)
            except:
                pass
                ...
                for part in url.split(' , '):
                    ...
                    if not d == '':
                        ...
                        if hmf.valid_url() == True: part = hmf.resolve()
                            ...
                            esres = es_resolvers.resolve_url(u)
                            if not esres == False: part = esres
                    urls.append(part)
    ...
    def getLanguage(self):
        langDict = {'Angol': ['en'], 'Magyar': ['hu'], 'Magyar+Angol': ['hu', 'en'], 'Német': ['de'], 'Magyar+Német': ['hu', 'de'], 'Francia': ['fr'], 'Magyar+Francia': ['hu', 'fr'], 'Spanyol': ['es'], 'Magyar+Spanyol': ['hu', 'es'], 'Olasz': ['it'], 'Magyar+Olasz': ['hu', 'it'], 'Lengyel': ['pl'], 'Magyar+Lengyel': ['hu', 'pl'], 'Koreai': ['ko'], 'Magyar+Koreai': ['hu', 'ko'], 'Orosz': ['ru'], 'Magyar+Orosz': ['hu', 'ru'], 'Görög': ['gr'], 'Magyar+Görög': ['hu', 'gr']}
    ...
    def _getPrimaryLang(self):
        langDict = {'Angol': 'en', 'Magyar': 'hu', 'Magyar+Angol': 'hu', 'Német': 'de', 'Magyar+Német': 'hu', 'Francia': 'fr', 'Magyar+Francia': 'hu', 'Spanyol': 'es', 'Magyar+Spanyol': 'hu', 'Olasz': 'it', 'Magyar+Olasz': 'hu', 'Lengyel': 'pl', 'Magyar+Lengyel': 'hu', 'Koreai': 'ko', 'Magyar+Koreai': 'hu', 'Orosz': 'ru', 'Magyar+Orosz': 'hu', 'Görög': 'gr', 'Magyar+Görög': 'hu'}
        name = control.setting('providers.lang')
    ...
    def getConstants(self):
        self.itemProperty = 'plugin.video.exoshark.container.items'

        self.metaProperty = 'plugin.video.exoshark.container.meta'
        ...
        try:
            ...
            self.hostDict += ['filescdn.net', 'sendit.cloud', 'uploadhaven.com', 'megadrive.co']
        except:
            self.hostDict = []

        self.hostprDict = ['1fichier.com', 'oboom.com', 'rapidgator.net', 'rg.to', 'uploaded.net', 'uploaded.to', 'ul.to', 'filefactory.com', 'nitroflare.com', 'turbobit.net', 'uploadrocket.net', 'mega.nz', 'extmatrix.com']

        self.hostcapDict = ['hugefiles.net', 'kingfiles.net', 'openload.io', 'openload.co', 'oload.tv', 'thevideo.me', 'vidup.me', 'streamin.to', 'torba.se', 'flashx.tv', 'flashx.to']
