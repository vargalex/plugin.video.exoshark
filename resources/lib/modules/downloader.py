...
def done(title, dest, downloaded):
    ...
    if downloaded:
        text += '%s : %s' % (dest.rsplit(os.sep)[-1], u'[COLOR forestgreen]Let\u00F6lt\u00E9s sikeres[/COLOR]'.encode('utf-8'))
    else:
        text += '%s : %s' % (dest.rsplit(os.sep)[-1], u'[COLOR red]A let\u00F6lt\u00E9s nem siker\u00FClt[/COLOR]'.encode('utf-8'))
...
def doDownload(url, dest, title, image, headers):
    ...
    if not resp:
        xbmcgui.Dialog().ok(title, dest, u'A let\u00F6lt\u00E9s meghi\u00FAsult'.encode('utf-8'), u'Nem \u00E9rkezett v\u00E1lasz a szervert\u0151l'.encode('utf-8'))
        return
    ...
    if content < 1:
        xbmcgui.Dialog().ok(title, file, u'Ismeretlen f\u00E1jlm\u00E9ret'.encode('utf-8'), u'Nem siker\u00FClt a let\u00F6lt\u00E9s'.encode('utf-8'))
        return
    ...
    down_text1 = u'Let\u00F6lt\u00E9s j\u00F3v\u00E1hagy\u00E1sa'.encode('utf-8')
    down_text2 = u'A f\u00E1jl m\u00E9rete'.encode('utf-8')
    down_text3 = u'Folytatja a let\u00F6lt\u00E9st?'.encode('utf-8')
    down_text4 = u'Let\u00F6lt\u00E9s'.encode('utf-8')
    down_text5 = u'M\u00E9gsem'.encode('utf-8')
    down_text6 = u'Let\u00F6lt\u00E9si folyamat'.encode('utf-8')

    if xbmcgui.Dialog().yesno('%s - %s' % (title, down_text1), file, '%s %dMB' % (down_text2, mb), down_text3, down_text4, down_text5) == 1:
        return
    ...
    while True:
        ...
        if percent >= notify:
            xbmc.executebuiltin( "XBMC.Notification(%s,%s,%i,%s)" % ('%s - %s%% - %s' % (down_text6, str(percent), title), dest, 10000, image))
