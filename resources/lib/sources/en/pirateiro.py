...
class source:
    def __init__(self):
        ...
        self.search_link = '/torrents/?c%s=1&search=%s&orderby=tamanho'
    ...
    def searchShowPack(self, title, season, episode, query, category):
        try:
            ...
            url = self.search_link % (category, urllib.quote_plus(query))
            url = urlparse.urljoin(self.base_link, url)
            r = client.request(url)

            result = client.parseDOM(r, 'table', attrs={'class': 'torrenttable'})[0]
            result = client.parseDOM(result, 'tr')
            result = [(client.parseDOM(i, 'a', attrs={'title': '.+?magnet link'}, ret='href')[0], client.parseDOM(i, 'b')[0], i) for i in result[1:]]
            ...
            for item in items:
                try:
                    ...
                    streamitems = sorted(streamitems, key=lambda x: x[1], reverse = True)
                    url = streamitems[0][0]
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            category = '700' if 'tvshowtitle' in data else '300'
