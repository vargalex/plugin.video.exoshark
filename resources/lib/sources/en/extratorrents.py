...
class source:
    ...
    def searchShowPack(self, title, season, episode, query):
        try:
            ...
            result = [(client.parseDOM(i, 'a', attrs={'title': 'Magnet link'}, ret='href')[0], client.parseDOM(i, 'td', attrs={'class': 'tli'})[0], i) for i in result]
            result = [(i[0], client.parseDOM(i[1], 'a')[0], i[2]) for i in result]
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            if 'tvshowtitle' in data and control.setting('pmcached.providers') == 'true' and not self.pm_api_key == '':
                ...
                sources += self.searchShowPack(title, season, episode, seasonquery)
