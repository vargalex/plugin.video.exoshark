...
class source:
    def __init__(self):
        self.priority = 0
        self.language = ['en']
        self.domains = ['limetorrents.co', 'limetorrents.info']
        self.base_link = 'https://www.limetorrents.co'
        self.search_link = '/searchrss/%s/'
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            result = [(client.parseDOM(i, 'title')[0], client.parseDOM(i, 'enclosure', ret='url')[0], client.parseDOM(i, 'size')[0]) for i in result]
            ...
            for item in result:
                try:
                    name = item[0]
                    magnetlink = item[1].rsplit('/torrent/')[1].split('.torrent')[0]
