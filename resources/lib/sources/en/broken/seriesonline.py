...
class source:
    ...
    def searchMovie(self, title, year, aliases, headers):
        try:
            ...
            url = 'https://api.yesmovie.io/series/movie/search/%s?link_web=%s/' % (cleantitle.geturl(title), self.base_link)
            r = client.request(url, headers=headers, timeout='15')
            info_url = re.findall('data-url="([^"]+)"', r)[0]
            info_url = info_url + '?link_web=%s/' % self.base_link
            inf = client.request(info_url, headers=headers, timeout='15')
            yr = re.findall('>\s*(\d{4})\s*<', inf)[0] 
