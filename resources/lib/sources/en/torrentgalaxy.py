...
class source:
    def __init__(self):
        self.priority = 0
        self.language = ['en']
        self.domains = ['torrentgalaxy.org', 'torrentgalaxy.pw']
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            if 'tvshowtitle' in data:
                ...
                url = urlparse.urljoin(self.base_link, self.search_link % imdb_id)
            r = client.request(url)

            result = client.parseDOM(r, 'div', attrs={'class': 'tgxtable'})[0]
            result = client.parseDOM(result, 'div', attrs={'class': 'tgxtablerow clickable-row click'})
            result = [(client.parseDOM(i, 'a', ret='title')[0], re.findall('href="(magnet[^"]+)"', i)[0], i) for i in result]
            ...
            for item in result:
                try:
                    ...
                    size = ''
                    try:
                        size = re.findall('((?:\d+\.\d+|\d+\,\d+|\d+)\s*(?:GB|GiB|MB|MiB))', item[2])[0]
