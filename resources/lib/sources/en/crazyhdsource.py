...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            for item in items:
                try:
                    log_utils.log('>>>> %s TRACE item <<<<\n%s' % (__file__.upper().split('\\')[-1].split('.')[0], item), log_utils.LOGDEBUG)
                    name = item[0]
                    ...
                    if 'extmatrix.com' in url:
                        check_ep = re.findall('[\.|\(|\[|\s](\d{4}|(?:S|s)\d*(?:\.|)(?:E|e)\d*|S\d*)[\.|\)|\]|\s]', url)[0]
                        check_ep = check_ep.replace('.', '')
                        if not check_ep.lower() == y.lower(): continue
