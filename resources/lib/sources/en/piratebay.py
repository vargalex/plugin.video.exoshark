...
class source:
    def __init__(self):
        self.priority = 0
        self.language = ['en']
        self.domains = ['pirateproxy.app', 'thepiratebay.org' 'proxybay.xyz', 'pirateproxy.bet', 'theproxybay.net', 'thepiratebay.vip', 'tpbproxy.online']
        self.base_link = 'https://pirateproxy.bet'
        self.search_link = '/search/%s/0/99/200'
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            result = client.parseDOM(r, 'table', attrs={'id': 'searchResult'})[0]
            result = client.parseDOM(result, 'tr')
            result = [(client.parseDOM(i, 'div', attrs={'class': 'detName'})[0], re.findall('href="(magnet[^"]+)"', i)[0], i) for i in result[1:]]
            result = [(client.parseDOM(i[0], 'a')[0], i[1], i[2]) for i in result]
            ...
            for item in result:
                try:
                    ...
                    try:
                        size = re.sub('&nbsp;', ' ', item[2])
                        size = re.findall('((?:\d+\.\d+|\d+\,\d+|\d+)\s*(?:GB|GiB|MB|MiB))', size)[0]
