...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            url = self.base_link + '/%s/%s/' % (query[0], query)
            r = client.request(url)

            result = client.parseDOM(r, 'tbody')[0]
            result = client.parseDOM(result, 'tr')
            result = [i for i in result if 'href="magnet' in i]
            result = [(client.parseDOM(i, 'a', ret='href')[0], client.parseDOM(i, 'a', ret='title')[-1], i) for i in result]
