        ...
        try:
            ...
            for item in r:
                try:
                    ...
                    data = client.parseDOM(data, 'div', attrs={'class': 'news-text'})[0]
                    try: data = data.split('</iframe>')[-1]
                    except: pass
                    # data = re.findall('\s*<b><a href=.+?>(.+?)</b>.+?<u><b><a href="(.+?)".+?</a></b></u>',
                                      # data[0].content, re.DOTALL)
                    data = re.findall('\s*<b><a href=.+?>(.+?)</b>.+?<u><b><a href="(.+?)".+?</a></b></u>', data)
                    u = [(i[0], i[1], s) for i in data]
