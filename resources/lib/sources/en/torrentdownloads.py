...
class source:
    def __init__(self):
        ...
        self.search_link = '/rss.xml?new=1&type=search&cid=%s&search=%s'
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            category = '8' if 'tvshowtitle' in data else '4'

            url = urlparse.urljoin(self.base_link, self.search_link % (category, urllib.quote_plus(query)))
            r = client.request(url)

            result = client.parseDOM(r, 'item')
            result = [(client.parseDOM(i, 'title')[0], client.parseDOM(i, 'info_hash')[0], client.parseDOM(i, 'size')[0]) for i in result]
